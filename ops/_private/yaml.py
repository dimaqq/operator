...
@tracer.start_as_current_span('ops.yaml.safe_load')
def safe_load(stream: Union[str, TextIO]) -> Any:
    """Same as yaml.safe_load, but use fast C loader if available."""
    # FIXME isinstance(stream, TextIO) doesn't work in a whole bunch of unit tests
    # related: https://github.com/python/typeshed/issues/3225
    # FIXME this works at runtime, but pyright is unhappy
    # if not isinstance(stream, (io.TextIOWrapper, TextIO)):
    #    opentelemetry.trace.get_current_span().set_attribute('len', len(stream))
    try:
        opentelemetry.trace.get_current_span().set_attribute('len', len(stream))  # pyright: ignore[reportArgumentType]
    except TypeError:
        pass
...
@tracer.start_as_current_span('ops.yaml.safe_dump')
